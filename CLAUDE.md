...
Item ID: `pdpcameeghhppjbhecolnhdepceeldhl`. Version 0.1.3 is customer-live. The 0.1.4 candidate merged to master on 2026-09-06 (PR #1, merge commit `f46e95c`), was packaged from tag `cliphutch-v0.1.4-candidate-2026-09-06` (verify:source, package, verify:package all passed; archive SHA-256 `a21843ecd7103a6fcb10df8989c3ace3be4749596c2f002796183e7bdaacfe5e`), and **was submitted for Chrome Web Store review on 2026-09-07 with manual publish**. **Publish gate cleared 2026-09-07:** the production Worker was promoted to version `3fc0e789-9e45-4339-ad49-a9a8c8b6f2c2`, which serves `/v2/activate`, `/v2/status` and `/v2/deactivate` alongside legacy `/validate` (runbook: `cloudflare/operations/WORKER-ROLLBACK-AND-RECONCILIATION.md`). Remaining before customers see 0.1.4: Google's review approval, then the founder clicks Publish, then one live $35 purchase-and-refund canary. Waived by the founder for this release: the synthetic-row staging acceptance run, the refund-safe hotfix, the Chrome 116/current-stable manual matrix, accessibility passes, and the product-value task; a live $35 purchase-and-refund canary after publication replaces the Stripe test-mode rehearsal.
...
Open release work:
- **Backend first** — DONE 2026-09-07: Payment Link ID set for production; staging Worker `cliphutch-api-staging` deployed with placeholder secrets (no Stripe test objects, by founder decision); both refunded rows dispositioned `confirmed-full` with no repair needed; migrations 0003 and 0004 applied to production; v2 routes promoted and smoked. The refund-safe hotfix was waived; `dd889c50` at 0% is the refund-unsafe rollback target (pause the Stripe webhook before any rollback).
- **Exact candidate** — DONE 2026-09-06: tag `cliphutch-v0.1.4-candidate-2026-09-06`, archive verified and submitted.
- **Manual product gates** — waived by the founder for 0.1.4 (Chrome 116/current stable, MV3 eviction and delayed-alarm cases, NVDA/VoiceOver, five-person paid-value task).
- **Publication gates** — hosted privacy copy deployed 2026-09-07; CWS permission/privacy declarations pasted at submission; new 0.1.4 screenshots still to capture (optional, listing shows the old UI).
