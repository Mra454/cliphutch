...
| 9 | `hls-master-separate-audio-ts-offset/master.m3u8` | HLS master | MPEG-TS video plus separate MPEG-TS AAC audio with an audio start offset; download must preserve sync |
| 10 | `hls-master-separate-audio-ts-aes/master.m3u8` | HLS master | AES-128 MPEG-TS video plus separate AES-128 MPEG-TS AAC audio; download must produce one MP4 with sound |

Total fixture size: under 15 MB.

## Notes

- Section 5 ships with empty stub segments. It fires the right network requests but is not playable; that is intentional. Sections 6 and 7 contain real encrypted segments and 16-byte keys. Sections 8 through 10 contain real separate video and audio renditions.
